...
paths_to_keys = {
    # 'valid' : '/misc/kcgscratch1/ChoGroup/thien/projects/wsd/dataPreparer/sharingWeaver/datasetOneMil11-sampled/valid.key', #'/misc/kcgscratch1/ChoGroup/thien/projects/wsd/dataPreparer/sharingWeaver/dataset/valid.key',
...
def predict(ncp, corpus, wsdModel, dictionaries, embeddings, features, anchorMat, useBinaryFeatures, idx2eventType,
            idx2sense, outFile):
    if 'sense' in ncp:
...
def score(predFile, keyFile, ncp):
    if 'event' in ncp:
        proc = subprocess.Popen(['python', scoreScript['event'], predFile, keyFile], stdin=subprocess.PIPE,
...
def train(dataset_path='',
    ...
    paramFileName = paramFolder + '/' + generateParameterFileName(model, expected_features, contextLength, nhidden,
                                                                  conv_feature_map, conv_win_feature_map, lamb)
    ...
    embeddings['anchor'] = numpy.concatenate(
        [numpy.zeros((1, embeddings['anchor'].shape[1]), dtype='float32'), embeddings['anchor'][startAnchor:endAnchor]],
        axis=0)
    ...
    for e in xrange(nepochs):
        ...
        miniId = -1
        while True:
            if np.random.randint(0, 35) < cutoff:
                oneBatch, _ = trainDataEvent.next()
                if not oneBatch['isValid']:
                    if cutoff != 10:
                        break
                    else:
                        cutoff = 0
                        continue
                if anchorMat.ndim == 1:
                    anchorMat = numpy.zeros(oneBatch['word'].shape, dtype='int32')\
                        if features['anchor'] == 0 else numpy.zeros(
                ...
                oneBatch, _ = trainDataSense.next()
                if not oneBatch['isValid']:
                    if cutoff != 10:
                        break
                    else:
        ...
        for elu in evaluatingDataset:
            _perfs[elu] = predict(elu, evaluatingDataset[elu], wsdModel, dictionaries, embeddings, features, anchorMat,
                                  useBinaryFeatures, idx2eventType, idx2sense, folder + '/' + elu + '.pred' + str(e))
